...
fn positive_control_pinned_track_golden_is_accepted() {
    ...
    // The in-tree copy of that pin is GONE (STEP 3, David 2026-09-15: `TRACK_REFERENCE_MODEL_MLX`
    // had no production consumer — the pin already came from the contract's own `target` block via
    // `golden::reference_model_pin_from_contract`), so the corpus contract is the only source and
    // there is no second place left for it to disagree with.
    assert_eq!(pin.repository, MLX_REFERENCE_REPOSITORY);
    assert_eq!(pin.revision, MLX_REFERENCE_REVISION);
}

/// The two platform reference pins, as TEST DATA. They were `TRACK_REFERENCE_MODEL_{MLX,CUDA}`,
/// deleted in STEP 3 because no production path read them; the CUDA-pin controls below still need
/// a second, DIFFERENT pin to refuse against, so the two live here.
const MLX_REFERENCE_REPOSITORY: &str = "Vontra/Qwen3.8-Flash-Next-MLX-4bit-MTP";
const MLX_REFERENCE_REVISION: &str = "327c8a604de613b42f84ba5e6b796c0931e8aa3b";
const CUDA_REFERENCE_REPOSITORY: &str = "RadixArk/Qwen3.8-Flash-Next-NVFP4";
const CUDA_REFERENCE_REVISION: &str = "7b719225242aacd3dbd3f9407468c2ee9a9d2594";
...
fn cuda_platform_pin_controls() {
    ...
    let cuda_pin = ReferenceModelPin {
        repository: CUDA_REFERENCE_REPOSITORY.to_string(),
        revision: CUDA_REFERENCE_REVISION.to_string(),
    };
    ...
    assert!(
        err.contains(&format!(
            ...
            "the track pins {}@{}",
            CUDA_REFERENCE_REPOSITORY, CUDA_REFERENCE_REVISION
        )),
        "{err}"
    );
    // Positive control: the CUDA checkpoint under the CUDA pin.
    doc["model_provenance"] = serde_json::json!({"repository": CUDA_REFERENCE_REPOSITORY, "revision": CUDA_REFERENCE_REVISION});
    let fx = load(&doc, &cuda_pin).expect("the CUDA golden must be ACCEPTED under the CUDA pin");
    assert_eq!(
        fx.model_provenance.unwrap().repository,
        CUDA_REFERENCE_REPOSITORY
    );
