...
pub fn verify_model_provenance_pinned(
    ...
    if provenance.repository != pin.repository || provenance.revision != pin.revision {
        // The reference's own diagnostic is the prefix (decision-identical, Swift parity); the
        // tail NAMES the mismatch so a refusal is auditable without re-reading either side.
        return Err(invalid(format!(
            "correctness golden model_provenance does not match the pinned reference model: \
             golden names {}@{}, the track pins {}@{}",
            provenance.repository, provenance.revision, pin.repository, pin.revision
        )));
    }
    Ok(())
}
...
///
/// `identity` is the resolved [`TrackModelIdentity`] of the track this run scores under
/// ([`crate::contract::model_identity`]): its vocabulary bound is the range every token id in the
/// document must fall in, and its seed length is the arity the `benchmark` oracle block must
/// carry. The identity is per-TRACK, so one tree loads a golden of any declared track and refuses
/// a golden of another track by the model-type gate below.
///
...
pub fn load_golden_fixture(
    ...
    if let Some(gates) = &decoded.correctness_gates {
        validate_golden_correctness_gates(
            gates,
            &decoded.cases,
            required_prompt_tokens,
            identity.vocab_size,
    ...
    if let Some(benchmark) = &decoded.benchmark {
        validate_benchmark_golden(benchmark, identity)?;
    }
...
/// `vocab_size` is the TRACK's bound ([`TrackModelIdentity::vocab_size`]), never a global: the
/// range a token id must fall in is a property of the checkpoint the track pins.
fn validate_tokens(tokens: &[Token], field: &str, vocab_size: usize) -> Result<()> {
...
fn validate_golden_cases(
    ...
    for c in cases {
        ...
        validate_tokens(
            &c.prompt_tokens,
            &format!("{}.prompt_tokens", c.name),
            vocab_size,
        )?;
        validate_tokens(
            &c.expected_tokens,
            &format!("{}.expected_tokens", c.name),
            vocab_size,
        )?;
    }
    Ok(())
...
fn validate_golden_correctness_gates(
    ...
    validate_golden_anchor_cases(gates.anchor_cases(), vocab_size)?;
    validate_golden_free_run_cases(gates.free_run_cases(), required_prompt_tokens, vocab_size)?;
    validate_golden_behavior_cases(gates.behavior_cases(), vocab_size)?;
    validate_unique_layered_case_names(base_cases, gates)?;
    Ok(())
}

/// The skeleton all three correctness case validators share: every case name is valid, no name
/// repeats inside its kind, and then the kind's own checks run — in that order, per case, so a
/// fixture with a bad name in case 1 and a bad field in case 2 refuses on the name exactly as it
/// always did.
///
/// `kind` is the word the two refusals carry (`anchor`, `free-run`, `behavior`); `check` is the
/// per-kind body.
fn validate_named_cases<C>(
    cases: &[C],
    kind: &str,
    name_of: impl Fn(&C) -> &str,
    mut check: impl FnMut(&C) -> Result<()>,
) -> Result<()> {
    let mut names = std::collections::HashSet::new();
    for c in cases {
        let name = name_of(c);
        validate_case_name(name, &format!("correctness {kind} case name"))?;
        if !names.insert(name.to_string()) {
            return Err(invalid(format!(
                "duplicate correctness {kind} case name {name}"
            )));
        }
        check(c)?;
    }
...
/// A case's token list must be non-empty and within its kind's cap. `label` is the fully-qualified
/// field (`{case}.{field}`) the refusals name.
fn validate_token_count(tokens: &[Token], label: &str, maximum: usize) -> Result<()> {
    if tokens.is_empty() {
        return Err(invalid(format!("{label} must not be empty")));
    }
    if tokens.len() > maximum {
        return Err(invalid(format!(
            "{label} has {} tokens; maximum is {}",
            tokens.len(),
            maximum
        )));
    }
...
fn validate_golden_anchor_cases(cases: &[GoldenAnchorCase], vocab_size: usize) -> Result<()> {
    validate_named_cases(
        cases,
        "anchor",
        |c: &GoldenAnchorCase| c.name.as_str(),
        |c| {
            validate_token_count(
                &c.context_tokens,
                &format!("{}.context_tokens", c.name),
            ...
            if let Some(accepted) = &c.accepted_tokens {
                ...
                validate_tokens(accepted, &format!("{}.accepted_tokens", c.name), vocab_size)?;
            }
...
fn validate_golden_free_run_cases(
    ...
    validate_named_cases(
        cases,
        "free-run",
        |c: &GoldenFreeRunCase| c.name.as_str(),
        |c| {
            if c.prompt_tokens.len() != required_prompt_tokens {
                return Err(invalid(format!(
                    "{}.prompt_tokens has {} tokens; need exactly {}",
...
fn validate_golden_behavior_cases(cases: &[GoldenBehaviorCase], vocab_size: usize) -> Result<()> {
    validate_named_cases(
        cases,
        "behavior",
        |c: &GoldenBehaviorCase| c.name.as_str(),
        |c| {
            validate_token_count(
                &c.prompt_tokens,
                &format!("{}.prompt_tokens", c.name),
...
pub fn validate_benchmark_golden(
    benchmark: &BenchmarkGolden,
    identity: &TrackModelIdentity,
) -> Result<()> {
    if benchmark.prefill_prompt_tokens.len() != identity.seed_tokens {
        return Err(invalid(format!(
    ...
    if benchmark.decode_seed_tokens.len() != identity.seed_tokens {
        return Err(invalid(format!(
...
#[cfg(test)]
mod tests {
    /// The Qwen 3.8 125B-A6B row — this module's fixtures, and the loader-parity / fuzz corpora
    /// they mirror, are 125B-generated. Resolved through the ONE accessor so a fixture cannot
    /// drift from the `--contract` fixture's declared model shape.
    fn identity_125b() -> TrackModelIdentity {
        crate::constants::TrackModelIdentity::new("qwen4_exp_text", 248_320, 48, 1_024)
    }

    use super::*;
    use serde_json::{json, Value};

    /// EVERY REFUSAL THE THREE CASE VALIDATORS RAISE FROM THEIR SHARED SKELETON, PINNED BYTE FOR
    /// BYTE. `validate_named_cases` builds the name refusals from a `kind` word and
    /// `validate_token_count` builds the field refusals from a label and a cap, so the strings an
    /// operator reads are now assembled rather than written out three times. This test is the
    /// evidence that the assembled bytes are exactly the bytes each kind used to write.
    ///
    /// REVERT-PROOF: change a `kind` word, a label, an order of words, or a cap constant, and one
    /// of these twelve equalities goes red.
    #[test]
    fn the_three_case_kinds_refuse_in_exactly_these_words() {
        const VOCAB: usize = 10_000;
        fn msg(err: BenchError) -> String {
            err.to_string()
        }
        let anchor = |context: Vec<Token>, name: &str| GoldenAnchorCase {
            name: name.to_string(),
        ...
        let free_run = |expected: Vec<Token>, name: &str| GoldenFreeRunCase {
            name: name.to_string(),
            prompt_tokens: vec![1],
            expected_tokens: expected,
            exact_prefix_tokens: None,
        };
        let behavior = |prompt: Vec<Token>, name: &str| GoldenBehaviorCase {
            name: name.to_string(),
            prompt_tokens: prompt,
            accepted_token_sequences: vec![vec![1]],
            max_new_tokens: 1,
            semantic_prompt: None,
            semantic_answer_key: None,
            semantic_reference_answer: None,
            semantic_domain: None,
            semantic_subdomain: None,
        };

        // (1) The case-name gate — `validate_case_name`'s field word.
        assert_eq!(
            msg(validate_golden_anchor_cases(&[anchor(vec![1], " a")], VOCAB).unwrap_err()),
            "correctness anchor case name \" a\" must not have leading or trailing whitespace"
        );
        assert_eq!(
            msg(validate_golden_free_run_cases(&[free_run(vec![1], " a")], 1, VOCAB).unwrap_err()),
            "correctness free-run case name \" a\" must not have leading or trailing whitespace"
        );
        assert_eq!(
            msg(validate_golden_behavior_cases(&[behavior(vec![1], " a")], VOCAB).unwrap_err()),
            "correctness behavior case name \" a\" must not have leading or trailing whitespace"
        );

        // (2) The duplicate-name refusal, which names the kind and the offending name.
        assert_eq!(
            msg(
                validate_golden_anchor_cases(&[anchor(vec![1], "a"), anchor(vec![1], "a")], VOCAB)
                    .unwrap_err()
            ),
            "duplicate correctness anchor case name a"
        );
        assert_eq!(
            msg(validate_golden_free_run_cases(
                &[free_run(vec![1], "a"), free_run(vec![1], "a")],
                1,
                VOCAB
            )
            .unwrap_err()),
            "duplicate correctness free-run case name a"
        );
        assert_eq!(
            msg(validate_golden_behavior_cases(
                &[behavior(vec![1], "a"), behavior(vec![1], "a")],
                VOCAB
            )
            .unwrap_err()),
            "duplicate correctness behavior case name a"
        );

        // (3) The empty-field refusal — each kind's own field name.
        assert_eq!(
            msg(validate_golden_anchor_cases(&[anchor(vec![], "a")], VOCAB).unwrap_err()),
            "a.context_tokens must not be empty"
        );
        assert_eq!(
            msg(validate_golden_free_run_cases(&[free_run(vec![], "a")], 1, VOCAB).unwrap_err()),
            "a.expected_tokens must not be empty"
        );
        assert_eq!(
            msg(validate_golden_behavior_cases(&[behavior(vec![], "a")], VOCAB).unwrap_err()),
            "a.prompt_tokens must not be empty"
        );

        // (4) The over-cap refusal — each kind's own field name AND its own cap constant.
        let over = |n: usize| vec![1 as Token; n + 1];
        assert_eq!(
            msg(validate_golden_anchor_cases(
                &[anchor(over(CORRECTNESS_MAX_ANCHOR_CONTEXT_TOKENS), "a")],
                VOCAB
            )
            .unwrap_err()),
            "a.context_tokens has 1025 tokens; maximum is 1024"
        );
        assert_eq!(
            msg(validate_golden_free_run_cases(
                &[free_run(over(CORRECTNESS_MAX_FREE_RUN_STEPS), "a")],
                1,
                VOCAB
            )
            .unwrap_err()),
            "a.expected_tokens has 257 tokens; maximum is 256"
        );
        assert_eq!(
            msg(validate_golden_behavior_cases(
                &[behavior(over(CORRECTNESS_MAX_BEHAVIOR_PROMPT_TOKENS), "a")],
                VOCAB
            )
            .unwrap_err()),
            "a.prompt_tokens has 2049 tokens; maximum is 2048"
        );
    }
    ...
    fn load(v: &Value) -> Result<GoldenFixture> {
        ...
        load_golden_fixture(&bytes, 4, 3, &identity_125b(), None, None, None)
    }
    ...
    fn load_pinned(v: &Value, pin: &ReferenceModelPin) -> Result<GoldenFixture> {
        let bytes = serde_json::to_vec(v).unwrap();
        load_golden_fixture(&bytes, 4, 3, &identity_125b(), None, None, Some(pin))
    }
        ...
        for wrong in [
            ...
            assert!(
                matches!(
                    ...
                    BenchError::InvalidInput(m)
                        if m.starts_with("correctness golden model_provenance does not match the pinned reference model")
                ),
    ...
    fn required_model_type_is_reported_before_the_provenance_identity() {
        ...
        ok_type
            .as_object_mut()
            .unwrap()
            .insert("model_type".into(), json!("qwen4_exp_text"));
        ...
        assert!(matches!(&err, BenchError::InvalidInput(m)
            if m.starts_with("correctness golden model_provenance does not match the pinned reference model")
                && m.contains("NotTheOrganizer/Some-Other-Model-4bit")
                && m.contains(REF_REPO)));
    }
    ...
    fn loader_pin_param_enforces_before_parse() {
        ...
        let pinned =
            load_golden_fixture(&bytes, 4, 3, &identity_125b(), None, Some(&pin), None).unwrap();
        let unpinned =
            load_golden_fixture(&bytes, 4, 3, &identity_125b(), None, None, None).unwrap();
    ...
    fn required_model_type_mismatch_rejected() {
        ...
        .unwrap_err();
        assert!(
            matches!(err, BenchError::InvalidInput(m) if m.contains("expected qwen4_exp_text"))
        );
    }
    ...
    fn fixture_sha256_is_of_raw_bytes() {
        let v = minimal_doc(3, 4);
        let bytes = serde_json::to_vec(&v).unwrap();
        let fx = load_golden_fixture(&bytes, 4, 3, &identity_125b(), None, None, None).unwrap();
    ...
    // --- PER-TRACK MODEL IDENTITY ---
    //
    // ONE `main`, four tracks. Each track's golden must load under ITS OWN identity and be
    // REFUSED under any other track's — that is the whole point of keying the identity by
    // `track_id` instead of pinning it per release branch.

    /// Every track this tree declares an identity for. Named here so a new row cannot be added
    /// without deciding what its golden looks like.
    /// FOUR TRACK IDENTITIES, as TEST DATA. They were the per-track table's rows; the table is
    /// gone (a track fixture declares its own identity now), and the loader's behaviour over four
    /// DIFFERENT identities is still worth pinning — so the four live here, as the inputs of a
    /// loader test rather than as a production source anything can read.
    fn declared_identities() -> [TrackModelIdentity; 4] {
        [
            TrackModelIdentity::new("qwen3_5_text", 248_320, 64, 512),
            TrackModelIdentity::new("gemma4_text", 262_144, 64, 1_024),
            TrackModelIdentity::new("qwen4_exp_text", 248_320, 48, 1_024),
            TrackModelIdentity::new("qwen4_exp_text", 248_320, 48, 1_024),
        ]
    }

    /// The MINIMAL golden a track accepts: its own `model_type`, one case whose prompt is the
    /// track's seed length, and `expected_tokens` inside the track's vocabulary. Nothing else —
    /// no oracle, no gates — so the only thing that can move a verdict is the IDENTITY.
    fn minimal_golden_for(identity: &TrackModelIdentity) -> Vec<u8> {
        serde_json::to_vec(&json!({
            "version": 1,
            "model_type": identity.golden_model_type,
            "cases": [{
                "name": "case-a",
                "prompt_tokens": vec![1i64; identity.seed_tokens],
    ...
    fn load_under(bytes: &[u8], identity: &TrackModelIdentity) -> Result<GoldenFixture> {
        load_golden_fixture(
            bytes,
            64,
            identity.seed_tokens,
    ...
    #[test]
    fn each_track_loads_its_own_golden_and_refuses_every_other_tracks() {
        for own_identity in declared_identities() {
            let bytes = minimal_golden_for(&own_identity);
            load_under(&bytes, &own_identity).unwrap_or_else(|e| {
                panic!(
                    "{} must load its own golden: {e}",
                    own_identity.golden_model_type
                )
            });

            for other_identity in declared_identities() {
                if other_identity == own_identity {
                    // The two 125B platform tracks share one checkpoint architecture, so they
                    // share a row: a golden of one IS a golden of the other. Nothing to refuse.
                    continue;
                }
                let err = load_under(&bytes, &other_identity).expect_err(
                    "a golden of another track must be REFUSED, never silently accepted",
                );
                let msg = format!("{err}");
                assert!(
                    msg.contains(&own_identity.golden_model_type)
                        && msg.contains(&other_identity.golden_model_type),
    ...
    /// The sealed `metrics.num_layers` is a DETERMINISTIC parity field: benchd and the reference
    /// score.json must agree on it byte-for-byte. Pin the LITERAL layer count of every identity the
    /// loader tests exercise, so an edit to the test data — or to the field the run seals it from —
    /// fails here instead of silently changing a value the parity roster compares.
    #[test]
    fn every_identitys_layer_count_is_pinned_to_its_checkpoint() {
        let layers: Vec<i64> = declared_identities()
            .iter()
            .map(|i| i.num_hidden_layers)
            .collect();
        assert_eq!(layers, vec![64, 64, 48, 48]);
    }

    #[test]
    fn the_vocabulary_bound_is_the_tracks_own_not_a_global() {
        // 248_320 is INSIDE the gemma vocabulary (262_144) and OUTSIDE both Qwen tracks'.
        let gemma = TrackModelIdentity::new("gemma4_text", 262_144, 64, 1_024);
        let qwen125b = TrackModelIdentity::new("qwen4_exp_text", 248_320, 48, 1_024);
        assert!(gemma.vocab_size > qwen125b.vocab_size);

        let mut doc = json!({
            "version": 1,
            "model_type": gemma.golden_model_type,
            "cases": [{
                "name": "case-a",
                "prompt_tokens": vec![1i64; gemma.seed_tokens],
                "expected_tokens": vec![qwen125b.vocab_size as i64; 64],
            }],
        });
        let bytes = serde_json::to_vec(&doc).unwrap();
        load_under(&bytes, &gemma).expect("a token inside the GEMMA vocabulary is accepted");

        // NEGATIVE CONTROL: the same id, one above the gemma bound, is refused by gemma too — so
        // the test above proves a BOUND, not that the check is absent.
        doc["cases"][0]["expected_tokens"] = json!(vec![gemma.vocab_size as i64; 64]);
        let over = serde_json::to_vec(&doc).unwrap();
        let err = load_under(&over, &gemma).expect_err("above the track's own bound is refused");
        assert!(
