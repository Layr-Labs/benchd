...
use crate::constants::TrackModelIdentity;
...
/// * every token id inside `0..<identity.vocab_size` — the TRACK's bound (same range check the
///   golden loader applies;
...
pub fn load_timed_prompt_tape(
    ...
    validate_tokens(&decoded.seed_tokens, "seed_tokens", identity.vocab_size)?;
    validate_tokens(
    ...
    validate_tokens(&row_chain, "rows[].sequential_argmax", identity.vocab_size)?;

    if let Some(emitted) = &decoded.emitted_tokens {
        validate_tokens(emitted, "emitted_tokens", identity.vocab_size)?;
...
pub fn load_timed_prompt_tape_from_path(
    ...
    load_timed_prompt_tape(&bytes, identity, pin)
}
...
mod tests {
    ...
    /// The Qwen 3.8 125B-A6B row, resolved through the ONE accessor rather than restated: the
    /// identity these synthesized tapes are written to (every invented id sits inside its
    /// vocabulary). A fixture named through the table cannot drift from it.
    fn identity() -> TrackModelIdentity {
    ...
    fn schema_true_tape_loads_and_exposes_the_leg_oracle() {
        ...
        let tape =
            load_timed_prompt_tape(&bytes, &identity(), None).expect("schema-true tape must load");
    ...
    fn optional_keys_may_be_absent() {
        ...
        let tape = load_timed_prompt_tape(&bytes_of(&doc), &identity(), None)
    ...
    fn pin_is_checked_on_raw_bytes_before_the_parse() {
        ...
        let good = load_timed_prompt_tape(&bytes, &identity(), None).unwrap();
        let pin = GoldenIntegrityPin {
            sha256: good.sha256.clone(),
            bytes: bytes.len() as u64,
        };
        assert!(load_timed_prompt_tape(&bytes, &identity(), Some(&pin)).is_ok());
        ...
        assert!(format!(
            "{}",
            load_timed_prompt_tape(&bytes, &identity(), Some(&wrong_bytes)).unwrap_err()
        )
        ...
        assert!(format!(
            "{}",
            load_timed_prompt_tape(b"{ [", &identity(), Some(&wrong_sha)).unwrap_err()
        )
    ...
    fn required_key_signatures_route_each_shape_and_are_disjoint() {
        ...
        assert!(load_timed_prompt_tape(&golden, &identity(), None).is_err());
        assert!(
            crate::golden::load_golden_fixture(&tape, 1, 1, &identity(), None, None, None).is_err()
        );
    }
    #[test]
    fn the_tape_vocabulary_bound_is_the_tracks_own() {
        // The tape loader takes the TRACK's identity, so the same bytes are judged against the
        // track's own vocabulary: an id at the Qwen bound is inside the GEMMA vocabulary and
        // outside the 125B one.
        let gemma = crate::constants::TrackModelIdentity::new("gemma4_text", 262_144, 64, 1_024);
        let qwen125b =
            crate::constants::TrackModelIdentity::new("qwen4_exp_text", 248_320, 48, 1_024);

        let mut doc = synth_tape(4, 4);
        doc["rows"][1]["sequential_argmax"] = json!(qwen125b.vocab_size as i64);
        doc["emitted_tokens"][1] = json!(qwen125b.vocab_size as i64);
        let bytes = bytes_of(&doc);

        load_timed_prompt_tape(&bytes, &gemma, None).expect("inside the gemma vocabulary");
        let err = load_timed_prompt_tape(&bytes, &qwen125b, None)
            .expect_err("outside the 125B vocabulary");
        assert!(
            format!("{err}").contains("outside configured vocab range"),
            "{err}"
        );
