...
//! shared windows (benchd `measure_job::shared_window_composite`, the SHARED-WINDOW ruling) — it
...
pub enum PerStreamAttestationError {
    ...
    /// [`Self::CapabilityAbsent`] so the two causes are not conflated in a refusal message.
...
/// benchd's measure-job as an additive diagnostic — the same struct the box-session-4
...
pub struct PerStreamAttestation {
    ...
    /// benchd's own parent clock (benchd `measure_job::shared_window_composite`).
...
/// The output of [`composite_diagnostic`] — same field shape as `benchd`'s
...
/// The published composite is benchd's `measure_job::shared_window_composite`, computed from the
...
mod tests {
    ...
    /// than imported, since this crate does not depend on `benchd` (where the certified
    /// `ScoredExponents`/`PREFILL_GAIN_EXPONENT`/`DECODE_GAIN_EXPONENT` actually live); the
    /// values themselves are the same ones `benchd`'s own `scored_exponents_certify_accepts_
