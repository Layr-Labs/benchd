...
mod tests {
    use super::*;
    use crate::constants::TRACK_ID;
    use crate::contract::{scored_regime, Contract};
    ...
    fn the_existing_track_is_report_only_and_no_refusal_can_fire() {
        // The regime the track's fixture declares — decode-only, which is what this inertness
        // proof is about. Resolved through the one resolver, from a fixture stating it, because
        // STEP 3 (David 2026-09-15) left no table for a silent fixture to fall back to.
        let regime = scored_regime(
            &Contract::parse(
