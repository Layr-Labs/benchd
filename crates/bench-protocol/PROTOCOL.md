...
and an **untrusted inference engine**: today the fork's generic `bench-worker` serving a
Runner on a Mac, a thin adapter over `ds4-resident` on a DGX Spark, or any third-party
engine that speaks this wire. This document and the accompanying JSON Schema
...
- **No cross-phase memoization / no repeated identical forwards.** Prefill, decode and
  correctness may share one worker process (the load-once window), so the isolation is the
  phase-start reset, not a new address space: the engine resets the KV cache and drains the
  allocator at the start of every phase, and the parent asserts the drain fail-closed at
  every phase boundary (see the allocator-drain rules above), so no model-owned memo may
  persist across phases; and no two identical forwards are issued inside one timed
...
| `free_decode_run` | `count` (= N) | `tokens[]` (len N), `acceptance_lengths[]`, `drafted_total`, `accepted_total`, `committed_total`, `verify_replay_disagreements`, `verification_mode`, `rectangular_verification_rounds`, `serial_verification_rounds` (optional) | engine free-runs its own MTP loop, commits N tokens, returns them + AUDIT counters. `verify_replay_disagreements` counts the rounds whose batched-verify argmax differed from the one-row replay argmax; the replay stands. It is OPTIONAL: absent means NOT REPORTED, not 0. When present it must be <= `drafted_total` - `accepted_total`, because only a rejecting round runs the replay |
...
## v1.2 additive extension (cohort form and the trusted-oracle replay)

Protocol **v1.2** is wire-additive on top of v1.1 and introduces no parallel verb family:
the batched path IS the general path and single-stream is `B = 1` of it.
`hello.protocol_version` stays `1`. Three capabilities gate it, each advertised before use;
benchd refuses to issue an unadvertised form as a protocol error, before the cool gate and
before the clock.

| capability | what it admits |
|---|---|
| `batched_free_run_decode` | the COHORT form of `free_decode_begin` / `free_decode_run`: the request carries `batch_size` and `seed_tokens_by_stream[]`; `count` is then N per stream; the response carries `seed_token_by_stream[]` / `effective_batch_size` and `tokens_by_stream[][]` (B x N) in place of `tokens` |
| `per_stream_timing` | the batched responses carry `prefill_ns_by_stream[]` / `decode_ns_by_stream[]`, per-slot monotonic nanoseconds. AUDIT only; benchd's own clock is the only score input |
| `cohort_reference_replay` | the `cohort_reference_replay` request kind: replay each cohort stream teacher-forced on the candidate's own committed tokens over the organizer's pinned reference weights and report the reference argmax per position. ONLY the organizer's trusted build advertises it; the untrusted candidate worker never serves it |

The Rust doc comments on `CAPABILITY_BATCHED_FREE_RUN_DECODE`,
`CAPABILITY_PER_STREAM_TIMING` and `CAPABILITY_COHORT_REFERENCE_REPLAY` in `src/lib.rs`
and the JSON Schema are the field-level definition. The 125B tracks are ruled single-stream
and do not use the cohort form.

## Runner identity on the `hello` (additive)

A worker that serves a **runner** (`docs/runner-contract.md`) adds one optional object
to the `hello`:

- `runner` — `{ "id", "model_type", "manifest_sha256", "build" }`. All four keys are
  required inside the object, and the object is closed. `id` is the manifest runner id.
  `model_type` is the `config.json` model type the worker loaded. `manifest_sha256` is
  the sha256 of the runner's canonical manifest JSON, in 64 lowercase hex characters.
  `build` is the worker build.

The field is appended last, so a worker that omits it round-trips unchanged. The field
is AUDIT and provenance only. The conformance kit compares the `hello` against the
declared manifest when you give it `--manifest`.

benchd records the four values in the `score.json` metrics, as `runner_id`,
`runner_model_type`, `runner_manifest_sha256` and `runner_build`. They are identity.
benchd never scores them. A key is absent when the `hello` did not carry it. benchd
never writes `0` or an empty string in its place.

## Resident identity on the `hello` (additive)

A worker can ATTACH to a process that already holds the loaded weights, instead of
loading the weights itself. The MLX `bench-worker resident` is that process. It is the
twin of `ds4-resident`. A worker that attached adds one optional object to the `hello`:

- `resident` — `{ "pid", "load_epoch" }`. Both keys are required inside the object, and
  the object is closed. `pid` is the resident process's id. `load_epoch` is a monotonic
  stamp. The resident process takes the stamp when its load ends.

The stamp stays the same for the life of the process. Every phase of one window
therefore seals the same identity. A value that changes is a reload. A reload must not
happen inside one window.

The field is appended last, after `runner`, so a worker that omits it round-trips
unchanged. A worker that loaded its own weights omits it. The field is AUDIT and
provenance only. It is not a manifest fact, so the conformance kit's manifest checks do
not read it.

benchd records the two values in the `score.json` metrics, as `resident_pid` and
`resident_load_epoch`. They are identity. benchd never scores them. A key is absent when
the `hello` did not carry it. benchd never writes `0` in its place.

A timed window can use more than one worker session. The official path spawns a new
worker for each timed phase. Every phase must report the SAME resident identity. benchd
refuses a window whose phases report different values, with the name
`resident_identity_changed_within_window`. A different value means the weights loaded
again inside the window, which is not permitted.
