...
pub struct MockEngine {
    ...
    /// Runner contract §6.1: the `runner` identity a runner-backed worker echoes on the `hello`.
    /// `None` (default) models a worker that serves no runner manifest.
    hello_runner: Option<bench_protocol::RunnerIdentity>,
    /// The `resident` identity a worker ATTACHED to a resident process echoes on the `hello`.
    /// `None` (default) models a worker that loaded the weights itself.
    hello_resident: Option<bench_protocol::ResidentIdentity>,
    ...
    /// #54: report a DIFFERENT `cache_memory` on the Nth (1-based) `phase_diagnostics` of the
    /// session, and `cache_memory_report` on every other one. Models a worker that drains at every
    /// phase boundary except ONE, so a test can pin WHICH boundary the parent's fail-closed
    /// assertion covers — the only way to tell the boundaries apart on a persistent window, where
    /// one session closes the warmup phases and both timed phases.
    cache_memory_on_nth: Option<(usize, i64)>,
    ...
    /// Spec-decode AUDIT: the `verify_replay_disagreements` counter to report on the single-stream
    /// `free_decode_run`. `None` OMITS the field entirely — the pre-field engine, which is the
    /// back-compat case, not a `0` report.
    free_run_verify_replay_disagreements: Option<u64>,
    /// Spec-decode AUDIT: the verify-path report (`verification_mode`, rectangular/serial round
    /// counts) on the single-stream `free_decode_run`. `None` OMITS all three fields.
    free_run_verification: Option<(String, u64, u64)>,
    ...
    stalling: bool,
    completed_work: i64,
    /// How many `phase_diagnostics` requests this session has received, 1-based once the current
    /// one is counted. Read by `cache_memory_on_nth`.
    phase_diagnostics_count: usize,
    ...
    /// and the teacher-forced decode-step inputs) — lets a test prove WHICH workload local-iterate
    /// times: the golden's benchmark oracle when present, else cases[0]'s stream (the scoped
    /// supersession of Ruling 1, `iterate.rs`).
...
pub struct RecordedTiming {
    ...
    /// The decode seed the timed decode worker received — from `decode_begin` (teacher-forced,
    /// e.g. `run_timed_benchmark`) OR `free_decode_begin` (the a8 free-run timed leg). Both carry
    /// the same `seed_tokens`, so one field records either verb.
    pub decode_seed: Vec<i64>,
    /// The teacher-forced per-step inputs (`decode_step` tokens). EMPTY under the a8 free-run timed
    /// leg, which issues no per-step calls — see `free_decode_run_count` for that leg's token budget.
    pub decode_step_inputs: Vec<i64>,
    /// The `count` a `free_decode_run` requested (the a8 free-run timed leg's N). `None` when the
    /// timed leg was teacher-forced (`decode_step` loop) rather than free-run.
    pub free_decode_run_count: Option<u32>,
}
...
impl MockEngine {
    ...
    fn queue_hello(&mut self) {
        let mut hello = WorkerResponse {
            ...
            // Runner contract §6.1: a runner-backed worker echoes its runner identity.
            runner: self.hello_runner.clone(),
            resident: self.hello_resident.clone(),
    ...
    /// Runner contract §6.1: echo this `runner` identity on the hello. Models a runner-backed
    /// worker; the default mock echoes none.
    pub fn hello_runner(mut self, runner: bench_protocol::RunnerIdentity) -> Self {
        self.hello_runner = Some(runner);
        self.outbox.clear();
        self.queue_hello();
        self
    }

    /// Echo this `resident` identity on the hello. Models a worker that ATTACHED to an
    /// already-loaded resident process; the default mock echoes none (it loaded its own weights).
    pub fn hello_resident(mut self, resident: bench_protocol::ResidentIdentity) -> Self {
        self.hello_resident = Some(resident);
        self.outbox.clear();
        self.queue_hello();
        self
    }
    ...
    /// Override the hello's `backend` and re-queue the hello. The default mock reports
    /// `"mock"`; a runner-backed worker reports its manifest backend.
    pub fn hello_backend(mut self, backend: &str) -> Self {
        self.hello_backend = Some(backend.to_string());
        self.outbox.clear();
        self.queue_hello();
        self
    }
    ...
    /// Report `verify_replay_disagreements` on the single-stream `free_decode_run`. Not calling
    /// this leaves the field OFF THE WIRE, which is the pre-field engine.
    pub fn free_run_verify_replay_disagreements(mut self, disagreements: u64) -> Self {
        self.free_run_verify_replay_disagreements = Some(disagreements);
        self
    }

    /// Report `verification_mode` + the rectangular/serial round counts on the single-stream
    /// `free_decode_run`. Not calling this leaves all three OFF THE WIRE (the pre-field engine).
    pub fn free_run_verification(mut self, mode: &str, rectangular: u64, serial: u64) -> Self {
        self.free_run_verification = Some((mode.to_string(), rectangular, serial));
        self
    }
    ...
    /// #54: report `bytes` on the `nth` (1-based) `phase_diagnostics` of this session and a
    /// conformant `0` on every other one. A persistent window closes four phases over ONE session
    /// (warmup prefill, warmup decode, timed prefill, timed decode), so a test names the boundary
    /// it is proving by its ordinal.
    pub fn cache_memory_on_nth_phase_close(mut self, nth: usize, bytes: i64) -> Self {
        self.cache_memory_report = Some(0);
        self.cache_memory_on_nth = Some((nth, bytes));
        self
    }
    ...
    fn effective_spec_echo(&self, req: &WorkerRequest) -> Option<SpecConfig> {
        // A `diverge_spec_echo` mock echoes its spec on EVERY seed forward, including one that
        // carried no request. That is not a contrived case: a resident serve started with
        // speculation on drafts whether or not benchd asked, and the echo is the only place it
        // shows. The calibration's serial-leg gate (`benchd::capture::refuse_spec_armed_engine`)
        // reads exactly this.
        ...
        match req.kind.as_str() {
            ...
            "free_decode_run" => {
                ...
                if let Some(b) = req.batch_size {
                    ...
                    resp.accepted_total = Some(accepted);
                    resp.committed_total = Some(committed);
                    resp.verify_replay_disagreements = self.free_run_verify_replay_disagreements;
                    if let Some((mode, r, se)) = &self.free_run_verification {
                        resp.verification_mode = Some(mode.clone());
                        resp.rectangular_verification_rounds = Some(*r);
                        resp.serial_verification_rounds = Some(*se);
                    }
                }
            }
            "correctness" => {
            ...
            "phase_diagnostics" => {
                ...
                // A per-ordinal override names ONE phase close as undrained and leaves the rest
                // conformant.
                resp.cache_memory = match self.cache_memory_on_nth {
                    Some((nth, bytes)) if nth == self.phase_diagnostics_count => Some(bytes),
                    _ => self.cache_memory_report,
                };
...
impl LineTransport for MockEngine {
    fn write_line(&mut self, line: &str) -> std::io::Result<()> {
        ...
        if kind == "phase_diagnostics" {
            ...
            self.phase_diagnostics_count += 1;
        ...
        if let Some(rec) = &self.recorded_timing {
            ...
            match kind.as_str() {
                ...
                // decode_begin (teacher-forced) and free_decode_begin (a8 free-run timed leg) both
                // carry the decode seed in `seed_tokens`; record either into `decode_seed`.
                ...
                // free_decode_run (a8 free-run timed leg): no per-step inputs, just the N budget.
                "free_decode_run" => {
                    r.free_decode_run_count = req.count;
                }
                _ => {}
            }
        }
