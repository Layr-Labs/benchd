...
//! `benchd measure-job` binary over `--preflight-only` (every pre-GPU check: golden load,
...
/// The track every contract in this file declares. Its MODEL IDENTITY — `model_type`, vocabulary
/// bound and seed length — is what every synthesized GoldenDocument here must carry: benchd loads
/// a golden under the identity the RUN'S TRACK declares, so a fixture written to another track's
/// constants is refused (that is the gate under test elsewhere, not an accident here).
const TRACK_ID: &str = "qwen3.8-27b-mtp-v1";

/// RE-PINNED (David 2026-09-15). [`bench_core::constants::TrackModelIdentity`] now OWNS its
/// `golden_model_type` as a `String` — the track fixture supplies that string — so the struct is
/// no longer `Copy` and a fresh identity per call would allocate at every use. Resolved ONCE and
/// handed out by reference; every call site reads the same identity it always did.
fn track_identity() -> &'static bench_core::constants::TrackModelIdentity {
    static IDENTITY: std::sync::OnceLock<bench_core::constants::TrackModelIdentity> =
        std::sync::OnceLock::new();
    IDENTITY.get_or_init(|| {
        bench_core::constants::TrackModelIdentity::new("qwen3_5_text", 248_320, 64, 512)
    })
}
...
/// The fully-parameterised builder: `model_type` (defaulting to whatever this tree REQUIRES —
/// named through the constant rather than restated, so the fixture cannot fall behind a model
/// migration) AND `model_provenance`, so a fixture can be wrong in one gate, the other, or BOTH
/// (case A8).
fn golden_document_json_full(
    ...
    let mut doc = serde_json::json!({
        ...
        "model_type": model_type
            .unwrap_or(&track_identity().golden_model_type),
        "cases": [{
            "name": "case-a",
            "prompt_tokens": vec![1i64; track_identity().seed_tokens],
            "expected_tokens": vec![2i64; 64],
        }],
        "benchmark": {
            "prefill_prompt_tokens": vec![1i64; track_identity().seed_tokens],
            "expected_prefill_token": 5,
            "decode_seed_tokens": vec![1i64; track_identity().seed_tokens],
...
fn golden_document_json_without_oracle() -> String {
    serde_json::to_string(&serde_json::json!({
        "version": 1,
        "model_type": track_identity().golden_model_type,
        "cases": [{
            "name": "case-a",
            "prompt_tokens": vec![1i64; track_identity().seed_tokens],
            "expected_tokens": vec![2i64; 64],
...
/// This suite's bed is [`bed::Fixture`] — the same four fields and the same constructor body the
/// two gate suites use, built WITHOUT a roster (this suite stages none). What stays here is what
/// this file is about: the contracts it pins and the goldens it synthesizes.
impl Fixture {
    ...
    ///
    /// ARMED, like every fixture in this file except the arm gate's own cases: these fixtures exist
    /// to exercise pins, windows, coverage and the seal, and an unarmed one would refuse them all
    /// at the arm gate first and hide what they are actually testing.
    fn contract_with_target(&self, pins: &[Pin], target: Option<serde_json::Value>) -> PathBuf {
        let mut extra = serde_json::json!({"official_scoring_enabled": true});
        ...
        self.contract_file(pins, "contract.json", extra)
    }
    ...
    fn contract_with_arm_state(&self, pins: &[Pin], armed: Option<bool>) -> PathBuf {
        ...
        self.contract_file(pins, "contract-arm.json", extra)
    }
    ...
    /// entirely (the state every other track's fixture is in). ARMED, so the mode fence is the only
    /// gate these cases can be stopped by before the bed's standing F-6 refusal.
    ///
    ...
    fn contract_with_allowed_modes(&self, pins: &[Pin], modes: Option<&[&str]>) -> PathBuf {
        ...
        self.contract_file(pins, "contract-modes.json", extra)
    }

    /// THE ONE CONTRACT WRITER: the well-formed, fully-pinned track fixture this file measures
    /// everything against, plus whatever `extra` top-level keys the case under test declares,
    /// written to `name`.
    ///
    /// The three builders above differed only in those two things — the extra key and the filename
    /// — while repeating the pool projection and the twelve-key fixture body verbatim, so a change
    /// to the fixture shape had to be made three times or the cases would silently disagree about
    /// what a well-formed contract is.
    fn contract_file(&self, pins: &[Pin], name: &str, extra: serde_json::Value) -> PathBuf {
        ...
        for (key, value) in extra.as_object().expect("extra keys are a JSON object") {
    ...
    /// Run `benchd measure-job --preflight-only` over the given goldens; returns (exit, stderr).
...
fn pinned_pool_tape_satisfies_the_golden_contract_offline() {
    let fx = Fixture::unrostered("tape-preflight", "ok");
...
/// never reach the live call. This drives the REAL `benchd measure-job` binary end to end: a
...
fn a_subset_of_the_pinned_pool_dies8_at_the_coverage_gate_call_site() {
    let fx = Fixture::unrostered("tape-preflight", "coverage-subset");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
...
fn scoring_seal_without_dispatch_record_dies8() {
    let fx = Fixture::unrostered("tape-preflight", "seal-absent");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract = fx.contract(&[pin(&tape_a, 0.9206), pin(&tape_b, 0.797)]);
    let ga = fx.golden("pool-alpha.json", &tape_a);
...
fn scoring_seal_with_disagreeing_proposed_commit_dies8() {
    let fx = Fixture::unrostered("tape-preflight", "seal-mismatch");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract = fx.contract(&[pin(&tape_a, 0.9206), pin(&tape_b, 0.797)]);
    let ga = fx.golden("pool-alpha.json", &tape_a);
...
fn legacy_golden_document_against_a_tape_pinned_pool_dies8_with_an_honest_diagnostic() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "legacy");
...
fn unpinned_tape_and_duplicate_digests_stay_die8() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "unpinned");
...
fn a_broken_tape_is_reported_as_a_broken_tape_not_as_a_bad_golden_document() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "broken");
...
fn a_pool_entry_whose_bytes_disagree_with_the_golden_dies8() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "bytes");
    let tape = tape_json(1, DECODE_STEPS);
...
fn a_tape_too_short_for_the_ruled_window_fails_preflight_and_a_live_length_tape_passes() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "short-window");
...
fn a_document_matching_neither_shape_names_both() {
    let fx = Fixture::unrostered("tape-preflight", "neither");
...
fn gates_routed_golden_without_the_benchmark_oracle_is_refused_naming_the_remedy_e2e() {
    let fx = Fixture::unrostered("tape-preflight", "no-oracle-e2e");
...
// The adversarial matrix. Each case drives the REAL `benchd measure-job --preflight-only`
...
fn a1_a_golden_with_no_provenance_is_untouched_by_the_pin() {
    let fx = Fixture::unrostered("tape-preflight", "a1");
...
fn a2_a_golden_naming_the_contracts_reference_model_passes_preflight() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "a2");
...
fn a3_a_golden_naming_a_different_model_than_the_contract_dies8_before_any_gpu_work() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "a3");
    let body =
...
fn a4_a_contract_declaring_half_a_reference_model_pin_refuses_the_run() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "a4");
    let body = golden_document_json_with_provenance(DECODE_STEPS, Some((REF_REPO, REF_REVISION)));
...
fn a5_a_target_that_declares_neither_half_refuses_the_run() {
    ...
    for (tag, target) in [
        ...
        let fx = Fixture::unrostered("tape-preflight", &format!("a5-{tag}"));
...
fn a6_a_contract_with_no_target_leaves_the_provenance_shape_checked_only() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "a6");
    let body =
        golden_document_json_with_provenance(DECODE_STEPS, Some((OTHER_REPO, OTHER_REVISION)));
    let contract = fx.contract(&[pin(&body, 0.9206)]);
...
fn a7_a8_the_model_type_gate_fires_before_the_provenance_identity() {
    ...
    for (tag, body) in [("a7", a7_body), ("a8", a8_body)] {
        let fx = Fixture::unrostered("tape-preflight", tag);
        let contract = fx.contract_pinning(&[pin(&body, 0.9206)], REF_REPO, REF_REVISION);
...
fn a9_a_placeholder_revision_is_a_contract_defect_not_a_golden_defect() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "a9");
    let body = golden_document_json_with_provenance(DECODE_STEPS, Some((REF_REPO, REF_REVISION)));
...
fn a10_a_padded_contract_value_is_a_contract_defect() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "a10");
    let body = golden_document_json_with_provenance(DECODE_STEPS, Some((REF_REPO, REF_REVISION)));
    let contract =
...
fn missing_calibration_under_enforcement_fails_pre_gpu() {
    ...
    let fx = Fixture::unrostered("tape-preflight", "f6-precal");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract = fx.contract(&[pin(&tape_a, 0.9206), pin(&tape_b, 0.797)]);
    let ga = fx.golden("pool-alpha.json", &tape_a);
...
// the REAL `benchd` binary on the MEASURING path (no `--preflight-only`) over an otherwise
...
fn unarmed_track_refuses_the_official_seal() {
    let fx = Fixture::unrostered("tape-preflight", "arm-false");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract =
...
fn armed_track_passes_the_arm_gate_untouched() {
    let fx = Fixture::unrostered("tape-preflight", "arm-true");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract =
...
fn contract_without_the_arm_flag_refuses_fail_closed() {
    let fx = Fixture::unrostered("tape-preflight", "arm-absent");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
...
fn local_dev_is_unaffected_by_an_unarmed_track() {
    let fx = Fixture::unrostered("tape-preflight", "arm-localdev");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract =
        fx.contract_with_arm_state(&[pin(&tape_a, 0.9206), pin(&tape_b, 0.797)], Some(false));
...
// read — so no fixture could speak for its own track. These cases drive the REAL `benchd` binary
...
fn contract_declared_dflash_mode_is_admitted() {
    let fx = Fixture::unrostered("tape-preflight", "modes-declared");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
...
fn undeclared_dflash_mode_is_refused_pre_gpu() {
    let fx = Fixture::unrostered("tape-preflight", "modes-absent");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract =
...
fn undeclared_contract_leaves_the_mtp_arm_unchanged() {
    let fx = Fixture::unrostered("tape-preflight", "modes-mtp-regression");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract =
        fx.contract_with_allowed_modes(&[pin(&tape_a, 0.9206), pin(&tape_b, 0.797)], None);
...
fn malformed_allowed_modes_is_a_fixture_refusal() {
    let fx = Fixture::unrostered("tape-preflight", "modes-malformed");
    let tape_a = tape_json(1, DECODE_STEPS);
    let tape_b = tape_json(2, DECODE_STEPS);
    let contract = fx.contract_with_allowed_modes(
        &[pin(&tape_a, 0.9206), pin(&tape_b, 0.797)],
