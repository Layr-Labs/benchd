...
//! 3.8 125B-A6B (MLX) track's declared model identity (steps=64 / seed=1024 /
//! model_type=qwen4_exp_text), which is why every invocation names that track AND hands the
//! command that track's fixture:
...
/// That track's FIXTURE — the only place the identity comes from since STEP 3 (David 2026-09-15).
fn track_contract() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests/fixtures/contract-full")
        .join(format!("{TRACK_125B}.json"))
}
...
fn run(args: &[&str]) -> i32 {
    let out = Command::new(env!("CARGO_BIN_EXE_benchd"))
        ...
        .args(["--contract", track_contract().to_str().unwrap()])
        .args(args)
