...
///
/// DELIBERATELY NOT folded into the single per-file content hash `crate::file_digest` now serves
/// the divergence gate and the workspace seal from. This walk — and [`walk_dir`] — never OPENS a
/// file: they read `stat` sizes only, over the `editablePaths` subtrees rather than the whole
/// workspace, so they are already at their floor and there is no read to share. The two also differ
/// from each other in ways a shared walker would have to re-introduce as options: [`walk_dir`] is
/// fail-closed on an unenumerable directory and short-circuits on the first cap overshoot in a
/// deterministic entry order; this one is best-effort and must complete to produce a sum. Merging
/// them would make a cheap check expensive and buy nothing measurable.
...
mod swift_parity {
    ...
    #[test]
    #[ignore = "compiles the pinned Swift enforcer with swiftc -O; run it with `cargo test -p benchd -- --ignored rust_and_swift_agree_on_every_shared_fixture`"]
