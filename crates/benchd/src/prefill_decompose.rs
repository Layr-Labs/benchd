//! `benchd prefill-decompose` — a bench-runner-level PREFILL-DECOMPOSITION diagnostic
//! (MLX item M-5, #68).
//!
...
//!
//! LIFECYCLE (documented choice): a FRESH engine process is spawned per SIZE, not per rep, so
//! each size's timing is measured on a cold process that has not inherited a warm
//! graph/allocator cache from an earlier size — while
...
pub const USAGE: &str = "\
benchd prefill-decompose — attribution-by-collapse of the single-shot prefill residual

USAGE:
    benchd prefill-decompose --engine <PATH> --weights <DIR> [OPTIONS]
...
pub fn run(args: &[String]) -> ExitCode {
    let parsed = match parse_args(args) {
        ...
        Err(msg) => {
            eprintln!("benchd prefill-decompose: {msg}");
            eprint!("{USAGE}");
    ...
    // Fresh engine per size: spawn a new `runtime-worker` child
    ...
    let measurements = match measure_sizes(&mut spawn, &parsed.sizes, parsed.reps) {
        Ok(m) => m,
        Err(e) => {
            eprintln!("benchd prefill-decompose: measurement failed: {e}");
    ...
    let fit = match least_squares_fit(&points) {
        ...
        None => {
            eprintln!(
                "benchd prefill-decompose: cannot fit a line (need ≥2 distinct sizes; got {})",
                parsed.sizes.len()
            );
            return ExitCode::from(1);
        }
