...
/// The contract's own `track_id`, which keys the per-track MODEL IDENTITY every pool tape is
/// loaded under. FAIL-CLOSED: a fixture with no `track_id` names no track, and a tape cannot be
/// validated against an identity nobody declared.
fn read_track_identity(
    contract_path: &Path,
) -> Result<bench_core::constants::TrackModelIdentity, String> {
    ...
    let track_id = loaded
        ...
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .ok_or("--contract declares no track_id; the pool tapes' model identity is per-track")?;
    // STEP 3 (David 2026-09-15) — the identity comes from the fixture itself, through the one
    // resolver, and an undeclared one refuses BY NAME. There is no per-track table left to read.
    bench_core::contract::model_identity(&loaded.contract, track_id).map(|r| r.value)
}
...
fn run_leg(
    ...
    // `coolgate::cool_gate_report`, which lives in the benchd BINARY crate and is not reachable
...
fn run() -> Result<(), String> {
    ...
    let identity = read_track_identity(&args.contract)?;
    ...
    for (idx, pin) in pins.iter().enumerate() {
        ...
        let tape = bench_core::tape::load_timed_prompt_tape_from_path(&path, &identity, None)
