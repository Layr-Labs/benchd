...
/// What one tracked entry IS, paired with a digest of what it CONTAINS. The kind distinguishes a
/// regular file from a SYMLINK so a file↔symlink swap of coincidentally-equal content still
...
/// same bytes.
///
/// The kind used to be a TAG BYTE folded into the digest (`sha256([kind] || bytes)`). It is a
/// separate field now, and the digest is the PLAIN `sha256(bytes)`, for one reason: a plain content
/// digest is the same number `crate::file_digest` computes for the workspace seal, so a file this
/// gate hashes is served from the process memo when the seal reaches it instead of being read a
/// second time. The comparison this feeds is pure EQUALITY between a baseline entry and a candidate
/// entry, and `(kind, sha256(bytes))` separates exactly the pairs `sha256([kind] || bytes)` did —
/// same kind and same bytes compare equal, anything else compares unequal. These values are
/// internal to the gate: none of them is emitted, sealed or pinned anywhere.
...
/// `sha256` of bytes already in hand — used for a symlink's TARGET STRING, which is the symlink's
/// tracked content and is never a file read.
...
/// Every REGULAR file AND SYMLINK under `root`, keyed by repo-relative slash path, valued by its
/// [`TrackedEntry`]. A symlink is tracked (not skipped) and NEVER traversed: git records it
...
///
/// TWO PHASES. The walk collects entries and reads only link targets; the regular files' bytes are
/// then hashed through [`crate::file_digest::sha256_files_parallel`] — the one per-file digest in
/// benchd. This replaced a SERIAL whole-file `fs::read` per entry, which on a ranked `measure-job`
/// read both workspaces' entire source surface on one thread, and then had the workspace seal read
/// the very same files again. Neither the node set nor any comparison changes: the walk is
/// unchanged, the parallel hash is per-file and order-independent, and the map is keyed by relative
/// path.
fn hash_tree(root: &Path) -> Result<BTreeMap<String, TrackedEntry>, String> {
    let mut out: BTreeMap<String, TrackedEntry> = BTreeMap::new();
    // Regular files, collected for the parallel hash below. Parallel to `file_rels`, which keeps
    // each one's repo-relative key (and lets a hashing failure name the file it failed on).
            ...
            if ft.is_symlink() {
                ...
                out.insert(
                    ...
                    TrackedEntry {
                        ...
                        content: content_digest(target.to_string_lossy().as_bytes()),
                    },
                );
            } else if ft.is_dir() {
                stack.push(path);
    ...
    let refs: Vec<&Path> = file_paths.iter().map(std::path::PathBuf::as_path).collect();
    let hashed = crate::file_digest::sha256_files_parallel(&refs, None).map_err(|e| {
        format!(
            "write-divergence: read {}: {}",
            file_paths[e.index].display(),
            e.error
        )
    })?;
    for (idx, content, _size) in hashed {
        out.insert(
            std::mem::take(&mut file_rels[idx]),
            TrackedEntry {
...
pub fn verify_no_write_outside_editable_from_git(
    ...
    let stream = crate::git_gate_stdout(
        candidate_root,
        &[
            "diff",
            "--name-status",
        ...
        " — --write-gate-base needs a git work tree and a git executable",
    )?;
...
mod tests {
    use super::*;
    // The ONE git driver for this crate's on-disk repo fixtures (see `crate::testgolden::git`).
    use crate::testgolden::git;
    ...
    /// THE CROSS-CONSUMER PROPERTY this refactor rests on: the gate and the workspace seal now
    /// share one per-file hasher and one process memo, and a tree the gate has already hashed must
    /// seal to EXACTLY the digest a cold process would compute. Run the gate first (warming the
    /// memo over every source file), then `dir_digest` the same tree, and compare against the same
    /// `dir_digest` taken with the memo cleared.
    #[test]
    fn the_workspace_seal_is_identical_whether_the_gate_warmed_the_memo_or_not() {
        let base = tmp("memo-base");
        let cand = tmp("memo-cand");
        for root in [&base, &cand] {
            write(root, "Package.swift", b"// pinned\n");
            write(root, "Sources/MLXFastCore/Timer.swift", b"trusted\n");
            write(root, "Sources/MLXFastModel/Head.swift", b"head\n");
            write(root, "benchmark.json", b"{}");
        }
        write(
            &cand,
        ...
        crate::file_digest::clear_memo_for_test();
        let cold = crate::iterate::dir_digest(&cand).unwrap();

        crate::file_digest::clear_memo_for_test();
        let m = br#"{"editablePaths":["Sources/MLXFastModel"]}"#;
        verify_no_write_outside_editable(m, &base, &cand).expect("edits are inside the surface");
        let warm = crate::iterate::dir_digest(&cand).unwrap();

        assert_eq!(
            warm, cold,
            "the memo must not change the sealed workspace digest"
        );
