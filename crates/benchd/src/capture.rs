...
//! ([`bench_core::contract::official_baseline`]), and a track with no captured pair is
...
//!
//! The record is the INPUT to a later, reviewed sentinel→value PR against the track fixture's
//! `official_baseline_{prefill,decode}_seconds_per_token`. It is not itself a baseline: nothing
//! reads it back.
//!
...
/// measurement).
/// WHERE A MULTI-PASS CAPTURE RUN WRITES: the per-label record base path, the labels the passes
/// carry (in pass order), and the identity every record is stamped with.
///
/// The three describe ONE destination — a label with no base path names no file, and a record with
/// no identity cannot say what produced its pair — so they travel as one value rather than as three
/// positional arguments beside the run's own inputs.
pub struct CaptureDestination<'a> {
    pub base: &'a Path,
    pub labels: &'a [String],
...
/// track whose fixture declares [`bench_core::contract::scores_against_live_control_leg`] stores
/// none — its ranked run measures
...
/// nothing can consume.
pub fn refuse_live_control_leg_track(
    contract: &bench_core::contract::Contract,
    track_id: &str,
) -> Result<(), String> {
    // CONTRACT-FIRST (David 2026-09-15): the fixture's own declaration, or a refusal by name.
    if bench_core::contract::scores_against_live_control_leg(contract, track_id)?.value {
        return Err(format!(
...
mod tests {
    ...
    /// A CAPTURED track refuses the capture mode, and a PENDING one opens it. The verdict is a
    /// pure function of "does this track have a pair?", and since STEP 3 (David 2026-09-15) that
    /// question is answered by the track fixture rather than by a per-track table — so the test
    /// states both answers directly instead of asking a table which tracks have one.
    #[test]
    fn the_capture_mode_opens_only_for_a_pending_track() {
        ...
        assert!(
            ...
            .is_ok(),
            "a track with no pair is exactly what the capture mode is for"
        );
    }
    ...
    fn capture_run_from_is_the_only_door_and_every_gate_is_on_it() {
        ...
        let run = capture_run_from(&healthy(Some(bench_protocol::SPEC_MODE_SERIAL), Some(0)))
