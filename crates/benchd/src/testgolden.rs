//! Test-only builder for the golden documents benchd's unit tests load.
//!
...
//! testing nothing.
//!
//! It is also where test support that more than ONE module needs lives, because it is the
//! crate's only `#[cfg(test)]` module: [`git`] is the single git driver for the on-disk repo
//! fixtures that `byte_budget` and `editable_divergence` both build.

use bench_core::constants::{
    AcceptanceBands, OfficialBaseline, BENCHMARK_DECODE_SEED_TOKENS, BENCHMARK_DECODE_STEPS,
    BENCHMARK_PREFILL_PROMPT_TOKENS, COHORT_TOKEN_TOLERANCE_PER_THOUSAND,
...
/// asymmetric decode band tight on regressions.
pub const TEST_BASELINE: OfficialBaseline = OfficialBaseline {
...
/// The Qwen 3.8 125B-A6B (MLX) row — the model identity every fixture this builder produces is
/// written to. Resolved through the ONE accessor
/// ([`bench_core::contract::model_identity`]) rather than restated, so the shared test golden
/// cannot drift from the `--contract` fixture's declared model shape.
/// BORROWED and resolved ONCE: the identity owns a `String` and is no longer `Copy`, and
/// `RunDigests` carries it by reference, so the shared helper hands out one long-lived value
/// rather than a fresh allocation per call site.
pub fn identity_125b() -> &'static bench_core::constants::TrackModelIdentity {
    static IDENTITY: std::sync::OnceLock<bench_core::constants::TrackModelIdentity> =
        std::sync::OnceLock::new();
    IDENTITY.get_or_init(|| {
        bench_core::constants::TrackModelIdentity::new("qwen4_exp_text", 248_320, 48, 1_024)
    })
}
use serde_json::json;

/// The `expected_prefill_token` / `expected_decode_seed_token` the standard [`benchmark_oracle`]
/// stamps. Exposed so a test that must configure a timed mock to RETURN this oracle (the a8/David
/// Option-A local-iterate timed leg verifies the free-run against `golden.benchmark`, not cases[0])
/// names the same values the block carries, rather than restating the literals.
pub const BENCHMARK_ORACLE_PREFILL_TOKEN: i64 = 5;
pub const BENCHMARK_ORACLE_SEED_TOKEN: i64 = 6;
...
/// prefill prompt and decode seed are the same workload in all of these fixtures. The decode window
/// is [`BENCHMARK_DECODE_STEPS`] long (the official / local-iterate scored window); the long
/// local-submit window uses [`benchmark_oracle_windowed`].
pub fn benchmark_oracle(prefill_fill: i64, decode_expected: i64) -> serde_json::Value {
    benchmark_oracle_windowed(prefill_fill, decode_expected, BENCHMARK_DECODE_STEPS)
}

/// [`benchmark_oracle`] with an explicit `expected_decode_tokens` length — for the local-submit
/// tests, whose 1023-step timed window (Option A now reads the benchmark oracle for it too) needs a
/// benchmark oracle at least that long. `decode_len` must be `>= BENCHMARK_DECODE_STEPS` for the
/// loader to accept the block.
pub fn benchmark_oracle_windowed(
    prefill_fill: i64,
    decode_expected: i64,
    decode_len: usize,
) -> serde_json::Value {
    json!({
        ...
        "expected_prefill_token": BENCHMARK_ORACLE_PREFILL_TOKEN,
        "decode_seed_tokens": vec![prefill_fill; BENCHMARK_DECODE_SEED_TOKENS],
        "expected_decode_seed_token": BENCHMARK_ORACLE_SEED_TOKEN,
        "expected_decode_tokens": vec![decode_expected; decode_len],
    })
}
...
impl TestGolden {
    ...
    /// The canonical all-`2` expected window corrupted at `index` PLUS enough further
    /// indices to BREAK the base-case mismatch budget (see [`corrupt_beyond_budget`]).
    /// This is how a base-case correctness-FAILURE test builds its golden since David's
    /// blanket-10% ruling (2026-09-02): one corrupt token is now inside the budget and
    /// PASSES. `index` stays the first mismatch.
    pub fn corrupt_expected_beyond_budget(self, index: usize, token: i64) -> Self {
        let mut expected = vec![2i64; self.required_steps];
        corrupt_beyond_budget(&mut expected, index, token);
        self.expected_tokens(expected)
    }
...
/// This RELEASE BRANCH's own track baseline, as TEST DATA.
///
/// It used to be resolved through the per-track table. The table is gone (STEP 3, David
/// 2026-09-15: the track fixture declares the pair and an absent declaration refuses), so the pair
/// lives here — at the full precision it was captured at, from the reference
/// `Constants.swift:255-256` @ `b26f76f` — as the ONE value every test in this crate that needs a
/// denominator reads. Its production counterpart is the fixture's declaration; the two agree
/// because `crates/benchd/tests/fixtures/contract-full/qwen3.8-27b-mtp-v1.json` carries the same
/// numbers and the oracle pins them.
pub fn track_fixture() -> bench_core::contract::Contract {
    bench_core::contract::Contract::parse(include_bytes!(
        "../tests/fixtures/contract-full/qwen3.8-27b-mtp-v1.json"
    ))
    .expect("this tree's own track fixture parses and certifies")
}

/// This RELEASE BRANCH's own track baseline, resolved from [`track_fixture`] through the ONE
/// resolver — the same path a run takes.
pub fn track_baseline() -> bench_core::constants::OfficialBaseline {
    bench_core::contract::official_baseline(&track_fixture(), bench_core::constants::TRACK_ID)
        .expect("this tree's own track fixture declares its captured pair")
        .value
}

/// Corrupt `tokens` at `first` and at enough LATER indices to break the per-case
/// teacher-forced mismatch budget (David's blanket-10% ruling, 2026-09-02:
/// `mismatches * 1000 <= COHORT_TOKEN_TOLERANCE_PER_THOUSAND * steps` over the
/// [`CORRECTNESS_STEPS`] window). `first` remains the FIRST mismatch, so a test keeps its
/// first-failing-step assertion; the padding indices are taken from the END of the
/// correctness window downward so they never precede `first`.
///
/// Used by BOTH shapes of failure test: the golden-side corruption
/// ([`TestGolden::corrupt_expected_beyond_budget`]) and the engine-side one (a mock's
/// teacher-forced token list).
pub fn corrupt_beyond_budget(tokens: &mut [i64], first: usize, token: i64) {
    let budget = (COHORT_TOKEN_TOLERANCE_PER_THOUSAND as usize * CORRECTNESS_STEPS) / 1000;
    tokens[first] = token;
    let mut corrupted = 1usize;
    let mut index = CORRECTNESS_STEPS;
    while corrupted <= budget && index > 0 {
        index -= 1;
        if index == first {
            continue;
        }
    ...
    assert!(
        corrupted > budget,
        "the correctness window is too short to break the mismatch budget"
    );
}
...
/// (the fork-point growth base) and `editable_divergence` (the fork-point write gate) each
/// carried a byte-identical copy, so a change to the pinned config had to be made twice or the
/// two fixture families would quietly build under different git settings.
///
/// Every invocation pins its own identity and turns SIGNING OFF (`commit.gpgsign=false`): the
/// fixtures must build identically on a laptop whose global config signs every commit and on a
/// runner with no key at all. `core.excludesFile=/dev/null` keeps a developer's global ignore
/// file from silently dropping a fixture file out of the commit the assertion — or the byte
/// count — depends on.
...
mod tests {
    ...
    fn steps_moves_the_document_and_the_loader_arity_together() {
        let li = crate::iterate::Mode::LocalIterate
            .golden_required_steps(&crate::iterate::test_window());
    ...
    fn required_steps_and_expected_fill_track_the_builder_arity() {
        ...
        let li = crate::iterate::Mode::LocalIterate
            .golden_required_steps(&crate::iterate::test_window());
        let submit =
            crate::iterate::Mode::LocalSubmit.golden_required_steps(&crate::iterate::test_window());
