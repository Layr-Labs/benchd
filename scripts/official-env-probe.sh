...
#
# Proves the benchd spawn delivers the SANITIZED, allowlist-only child environment to the live
...
# `MLXFAST_USE_RUNTIME_WORKER=0`) applies to ALL benchd spawns, official AND local.
#
...
# TOOL-ERR'd. The fix runs the probe against benchd's `iterate --mode local-iterate` spawn: local
...
#
# GPU-FREE: this loads no model (the shim never handshakes), so it needs the REAL benchd but NOT
...
#
# Env: BENCHD_BIN WEIGHTS · OFFICIAL_GOLDEN · OUT   (optional OFFICIAL_COMMIT)
set -uo pipefail
HERE="$(cd -P "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
. "$HERE/official-lib.sh"

: "${BENCHD_BIN:?set BENCHD_BIN}" "${WEIGHTS:?set WEIGHTS}" "${OFFICIAL_GOLDEN:?set OFFICIAL_GOLDEN}" "${OUT:?set OUT}"
mkdir -p "$OUT"
...
# Substituted as benchd's `--engine`. Dumps its FULL received env (the sanitized child env) with
...
# Do NOT handshake — benchd will error at the hello; that is expected and ignored. Under
# local-iterate the spawn is NOT sandboxed deny-write, so the write above SUCCEEDS.
exit 0
SHIM
chmod +x "$SHIM"

# --- sentinels planted in benchd's PARENT env, one per allowlist bucket ---------------
...
echo "official-env-probe: spawning benchd LOCAL-ITERATE with an env-dump worker shim ($SHIM_ABS)"
...
env \
  ...
  "$BENCHD_BIN" iterate \
    ...
    --contract "${MLXFAST_CONTRACT_PATH:?set MLXFAST_CONTRACT_PATH to the track fixture}" \
    --mode local-iterate \
    --score-path "$OUT/probe-score.json" \
  > "$OUT/benchd.stdout" 2> "$OUT/benchd.stderr" || true   # benchd exits non-zero at the hello — expected
...
if [ ! -s "$CHILD_ENV" ]; then
  ...
  echo "  benchd.stderr tail:" >&2; tail -5 "$OUT/benchd.stderr" 2>/dev/null | sed 's/^/    /' >&2
  exit 4
fi
...
echo ""
if [ "$FAIL" -eq 0 ]; then
  echo "official-env-probe: RESULT PASS — benchd local-iterate spawn delivered an allowlist-only child env (LC_ prefix + exact survive; MLXFAST_*/non-allowlisted dropped; guard forced 0). Same B-1 sanitizer as the official spawn; official deny-write is orthogonal + separately verified. #47 §9 evidence."
  exit 0
else
