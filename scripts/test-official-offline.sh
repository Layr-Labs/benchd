...
# goldens + a stub benchd / stub swift / stub probe-benchd (the compat-matrix / variant-offline
...
#      abort (exit 4). Uses a stub benchd that runs the env-dump shim under a chosen env.
#
# The REAL GPU steps (live benchd official + direct swift official; the live sandboxed spawn) are
...
# stub benchd: iterate (writes sealed score + .sha256 + 9-field integrity), parity-diff, validate-golden.
STUB_BC="$WORK/bc.sh"; cat > "$STUB_BC" <<'EOF'
#!/bin/bash
sub="$1"; shift
case "$sub" in
  --version) echo "benchd vSTUB"; exit 0;;
  ...
  iterate)
    ...
    if [ "${STUB_BC_DROP_RUNNER:-}" = 1 ]; then runner=""; else
      runner=',"candidate_executable":"/stub/engine","candidate_executable_sha256":"stubc","baseline_executable":"","baseline_executable_sha256":"","candidate_executable_resolution":"canonical","benchd_executable":"/stub/benchd","benchd_executable_sha256":"stubb","candidate_workspace_sha256":"","sandbox":"seatbelt"'
    fi
...
if [ "$HAVE_JQ" = 1 ]; then
  ...
  # DIFF_CMD is intentionally NOT set — official-parity defaults it to "$BENCHD_BIN parity-diff"
  ...
  COMMON="HOME=$WORK BENCHD_BIN=$STUB_BC ENGINE=/bin/echo SWIFT=$STUB_SW WEIGHTS=$WORK/wdir OFFICIAL_GOLDEN=$WORK/official.json OFFICIAL_COMMIT=deadbeefcafe OFFICIAL_LIB=$HERE/official-lib.sh"
  ...
  FMCOMMON="HOME=$WORK BENCHD_BIN=$STUB_BC ENGINE=/bin/echo SWIFT=$STUB_SW WEIGHTS=$WORK/wdir GEN=$HERE/gen-failure-corpus.py OFFICIAL_COMMIT=deadbeefcafe OFFICIAL_LIB=$HERE/official-lib.sh"
  ...
  # 3b. harness-failure guard: benchd oracle passes → ABORT (exit 5).
...
# stub benchd for the probe: models the LOCAL-ITERATE spawn — it applies B-1's env sanitization
...
# The probe passes `--mode local-iterate` and `--contract` (STEP 3: benchd holds no per-track table,
# so every mode resolves its window and model shape from the track fixture); the stub ignores both —
# the sanitizer is what matters. The probe is given this tree's own reference fixture.
PROBE_CONTRACT="$HERE/../crates/benchd/tests/fixtures/contract-full/qwen3.8-125b-a6b-mlx-v1.json"
...
exit 1   # benchd "fails at hello" — the probe ignores this
EOF
chmod +x "$STUB_BCP"

# 4a. good (sanitized) capture → PASS.
if env BENCHD_BIN="$STUB_BCP" WEIGHTS="$WORK/wdir" OFFICIAL_GOLDEN="$WORK/official.json" STUB_PROBE_MODE=good MLXFAST_CONTRACT_PATH="$PROBE_CONTRACT" \
...
# 4b. leaked env → FAIL.
if env BENCHD_BIN="$STUB_BCP" WEIGHTS="$WORK/wdir" OFFICIAL_GOLDEN="$WORK/official.json" STUB_PROBE_MODE=leaked MLXFAST_CONTRACT_PATH="$PROBE_CONTRACT" \
...
if env BENCHD_BIN="$STUB_BCP" WEIGHTS="$WORK/wdir" OFFICIAL_GOLDEN="$WORK/official.json" STUB_PROBE_MODE=nospawn MLXFAST_CONTRACT_PATH="$PROBE_CONTRACT" \
