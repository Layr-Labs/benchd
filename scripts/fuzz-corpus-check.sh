...
#   (2) BENCHD-SIDE loader verdicts: run `benchd validate-golden` on every fixture and assert
...
# hardened harness scripts/loader-parity.sh (benchd validate-golden vs mlxfast-swift preflight,
# agree-or-declared), the SAME harness the §6 loader-parity corpus uses. On the box:
#
#   BENCHD_BIN=target/release/benchd SWIFT=/path/to/mlxfast-swift WEIGHTS=/path/to/weights \
...
# TRACK — validate-golden judges a golden against a per-TRACK model identity (model_type,
# vocabulary bound, seed length). This corpus was GENERATED under the Qwen 3.8 125B-A6B tower
# (scripts/gen-fuzz-corpus.py), which is the default. Override with FUZZ_CHECK_TRACK when
# pointing the check at a corpus generated under another track.
TRACK="${FUZZ_CHECK_TRACK:-qwen3.8-125b-a6b-mlx-v1}"
BENCHD_BIN="${BENCHD_BIN:-$ROOT/target/debug/benchd}"
# REPORT — a RUN OUTPUT, so it lands in the ignored .parity/ evidence directory with the
# other local run evidence. It must never point inside docs/: docs/history/ is frozen.
REPORT="${REPORT:-$ROOT/.parity/fuzz-corpus-report.txt}"
...
[ -x "$BENCHD_BIN" ] || { echo "fuzz-check: FATAL benchd not executable: $BENCHD_BIN (cargo build -p benchd)" >&2; exit 2; }
mkdir -p "$(dirname "$REPORT")" || { echo "fuzz-check: FATAL cannot create the report directory" >&2; exit 2; }
...
{
  echo "# M-4 fuzz-corpus check — FREEZE pin + benchd (Rust) loader verdicts"
  echo "corpus:   ${CORPUS#$ROOT/}"
  echo "manifest: ${MANIFEST#$ROOT/}"
  echo "benchd: ${BENCHD_BIN#$ROOT/}"
  echo "fixtures: $COUNT"
  echo ""
  printf "%-40s %-8s %-8s %-9s %s\n" FIXTURE EXPECT BENCHD FREEZE SWIFT
} > "$REPORT"
...
for row in "${ROWS[@]}"; do
  ...
  "$BENCHD_BIN" validate-golden --golden "$fx" --track "$TRACK" ${CONTRACT_ARGS[@]+"${CONTRACT_ARGS[@]}"} >/dev/null 2>&1; act="$(decide $?)"
...

{
  ...
  echo "         freeze-drift=$freeze_bad  benchd-verdict-mismatch=$verdict_bad"
  if [ "$freeze_bad" -eq 0 ] && [ "$verdict_bad" -eq 0 ]; then
    echo "RESULT: PASS — corpus frozen (all sha256+bytes pinned) and benchd verdicts stable"
  else
...
# hardened harness so the Swift verdicts are compared agree-or-declared against benchd.
if [ -n "${SWIFT:-}" ] && [ -n "${WEIGHTS:-}" ]; then
  echo ""
  echo "fuzz-check: SWIFT + WEIGHTS set -> running full dual-loader parity via loader-parity.sh"
  BENCHD_BIN="$BENCHD_BIN" SWIFT="$SWIFT" WEIGHTS="$WEIGHTS" \
    "$ROOT/scripts/loader-parity.sh" "$CORPUS" "${DUAL_REPORT:-$ROOT/.parity/fuzz-dual-loader-report.txt}" || status=1
else
  echo ""
  ...
  echo "    BENCHD_BIN=target/release/benchd SWIFT=<mlxfast-swift> WEIGHTS=<weights-dir> \\"
  echo "      scripts/loader-parity.sh $CORPUS .parity/fuzz-dual-loader-report.txt"
fi
exit "$status"
