#!/usr/bin/env bash
# WS1-10 — M1 parity: benchd (Rust) vs benchmark.sh (Swift) on identical
...
# CHAL = the Swift REFERENCE checkout. Now the live engine repo; the mlxfast-challenge-dev
# fork it was named for is RETIRED (the variable name is kept to avoid churn).
CHAL="$ROOT/mlxfast-qwen-38-27b-mtp-engine"
ENGINE_BIN="$ROOT/mlxfast-engine/.build/release/mlxfast-engine"
SWIFT_BIN="$CHAL/.build/release/mlxfast-swift"
BENCHD_BIN="$ROOT/mlxfast-bench/target/release/benchd"
# EXPORT so the parity-diff.py shim (§T4) resolves this benchd via $BENCHD_BIN instead of
...
SCORE_RUST="$WORK/score.benchd.json"
...
# mirrors that. On the benchd side the pair is per-TRACK, not global: it is the entry this tree's
# track (bench_core::constants::TRACK_ID) has in the per-track baseline table, read through
# bench_core::constants::official_baseline and pinned against the reference source by
...
# RETIRED mlxfast-challenge-dev fork's constants — so the driver was quietly feeding benchd a denominator
...
main() {
  need "$ENGINE_BIN"; need "$SWIFT_BIN"; need "$BENCHD_BIN"
  ...
  say "Rust candidate: benchd iterate (drives mlxfast-engine)"
  # `--contract` is required on every mode (STEP 3, David 2026-09-15): benchd holds no per-track
  # table, so the window, the model shape and the scoring rules all come from the track fixture.
  "$BENCHD_BIN" iterate --engine "$ENGINE_BIN" --weights "$WEIGHTS" --golden "$GOLDEN" \
