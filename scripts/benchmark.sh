...
#     heredoc and the golden-file-not-found two-liner (verbatim strings).
...
if [[ "${OFFICIAL}" == "1" ]]; then
  ...
  # ARM GATE (David 2026-08-26): the official/scored seal refuses unless the track fixture declares
  # official_scoring_enabled: true. The fixture path comes from MLXFAST_CONTRACT_PATH; a missing or
  # unset contract fails closed inside benchd (an absent arm state is not an armed one).
  CONTRACT_PATH="${MLXFAST_CONTRACT_PATH:-}"
  if [[ -z "${CONTRACT_PATH}" ]]; then
    echo "benchmark.sh: official runs require MLXFAST_CONTRACT_PATH (the track fixture whose official_scoring_enabled arms scoring)" >&2
    exit 1
  fi
  if [[ ! -f "${CONTRACT_PATH}" ]]; then
    echo "benchmark.sh: track fixture not found at ${CONTRACT_PATH} (MLXFAST_CONTRACT_PATH)" >&2
    exit 1
  fi
  benchd_args+=(--contract "${CONTRACT_PATH}")
else
  # STEP 3 of the constants→contract migration (David 2026-09-15): benchd holds no per-track
  # table, so EVERY mode resolves its window, model shape, floors, bands and weights from the
  # track fixture. A local run without one refuses by name inside benchd; refuse here instead,
  # with the variable to set.
  CONTRACT_PATH="${MLXFAST_CONTRACT_PATH:-}"
  if [[ -z "${CONTRACT_PATH}" ]]; then
    echo "benchmark.sh: local runs require MLXFAST_CONTRACT_PATH (the track fixture declaring the window, the model shape and the scoring rules)" >&2
    exit 1
  fi
  if [[ ! -f "${CONTRACT_PATH}" ]]; then
    echo "benchmark.sh: track fixture not found at ${CONTRACT_PATH} (MLXFAST_CONTRACT_PATH)" >&2
  ...
  benchd_args+=(--contract "${CONTRACT_PATH}" --cool-gate)
fi
...
rc=0
"${BENCHD_BIN}" iterate "${benchd_args[@]}" || rc=$?
