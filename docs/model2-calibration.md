...
document as their contract. Every mechanism claim below is grounded in the repo at
`path.rs:line`.

What remains open is DATA, not design: the bands themselves are NOT authored here — they are
MEASURED on the box per §4, and that authoring leg is still to be run.
...
> frontier `1.376`, the native calibration values (expected raw median `0.9940390645`, band
> `±2.0%` — track fixture `calibration.expected_raw_median` / `calibration.band_pct`), and any
> pre-existing BASELINE_CALIBRATION serial band authored under the wrapper are
> **native-regime artifacts**.
...
run (candidate == serial ⇒ ratio 1.0 ⇒ median 1.0) scores exactly `1.0`. Floor and ceiling
edges are inclusive: the floor compares with `<` and the ceiling with `>`.
...
- **File shape** — `crates/benchd/src/measure_job.rs:848` `BaselineCalibration`:
...
- **Resolution** — `crates/benchd/src/measure_job.rs:911` `BaselineCalibration::resolve`:
...
- **Two REQUIRED file-wide fields.** `timed_mode` (the series the means were measured in) and
  ...
  `crates/benchctl/src/measure_job.rs@ffea0d3:1100` and `:1105` (the crate carried the name
  `benchctl` at that sha), both plain `String` with no `#[serde(default)]`. They are file-wide
  identity, not per-target: one file holds one series and one track.
...
under the raw serial-anchored semantics. In the native regime this is `0.9940390645` ± `2.0%`
(track fixture `calibration.expected_raw_median` / `calibration.band_pct` — parity data
measured on the native wrapper over six gated sessions, **NOT re-derived against benchd's
clock**).

- **Predicate** — none in this tree. benchd models no `calibration` block: the contract schema
  ignores that key, so nothing reads `expected_raw_median` or `band_pct`
  (`crates/bench-core/src/contract.rs`). The envelope is a record of what the native wrapper
  measured. It gates no run.

**Model-2 adaptation.** The stock-tree expected raw median must be re-measured under benchd's
clock, a Model-2 `expected`/`band_pct` established, and a predicate written to read them. Until
then this envelope is report-only (no native `0.9940…` value gates a Model-2 run).
...
3. **Authoring** — the band is authored ONLY off a fully-accepted, parity-true run:
   `crates/benchd/src/measure_job.rs:1171` `should_author_bootstrap(candidate_accepted,
...
2. **Wire-seam repair + real handshake.** Model-2 bands are measured on the box only AFTER
   ...
   - spec-never-ignored echo — `measure_free_run_decode` (`:701`
     ...
     (`crates/benchd/src/measure_job.rs:1341` `EffectiveSpec`).
...
> expected-raw-median envelope is report-only for Model-2. The other Model-2
...
(`RUN_TIMEOUT_MARGIN = 4.0`, `crates/bench-core/src/constants.rs:263`; fallback per-token
ceiling `:278` when no `BASELINE_CALIBRATION` is available, e.g. the free-run path). On timeout
