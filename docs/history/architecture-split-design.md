> **HISTORY.** This is the original split design, superseded as the live description on
> 2026-09-08 by [`../architecture.md`](../architecture.md). Kept for the reasoning: the
> red/green cycles (§8) and the deliverables that were never built (`target.toml`, the benchd
> container image, `bench-agent`, the TCP bridge). Cite it `@sha`-pinned; it does not describe
> the system as it runs. Until 2026-09-08 this file lived at `docs/architecture.md`, so a pin
> such as `docs/architecture.md@06ba6a8 §3` resolves to this text.

# Splitting the benchmarker from the inference engine

**The architecture of `mlxfast-bench` — SHIPPED**

A Rust, reproducible, sandboxed benchmarker; interchangeable inference engines behind
a frozen wire protocol — Swift/MLX raw on M5s today, CUDA next.

This began as a proposal against the (now retired) single-repo `mlxfast-challenge-dev`
shape. §§1-6 and §8 describe what was built and remain the live description. §7 (the
migration plan) has EXECUTED and §9's open questions have been resolved or ruled;
both are marked in place rather than deleted, so the reasoning stays readable.
...
```mermaid
flowchart LR
    subgraph REPO ["the retired single-repo shape · one repo, one binary"]
...
flowchart LR
    subgraph TRUSTED ["benchd · Rust, Docker, pinned digest"]
        CLI[benchd CLI] --> RUN[runner<br/>spawn + wall-clock timing]
...
3. **Semantic (golden conformance) — the real definition.** Metal and CUDA cannot
   ...
   primary teacher-forced base cases are NOT exact-match: a base case passes if the
   not-accepted tokens stay in the blanket 10% budget (`mismatches * 1000 <=
   COHORT_TOKEN_TOLERANCE_PER_THOUSAND * steps`, integer arithmetic, the same rule the
   batched cohort path applies). The TIMED free-run oracle stays exact. **The
...
>
> Give the kit a runner manifest with `--manifest <PATH>`, and it also checks the
> `hello` against that manifest before the gates run. The manifest declares the
> decoders and the regimes; the worker derives the `hello` from it. The kit checks
> that the advertised spec modes are decoders the manifest declares, that the
> capabilities and the cohort ceiling are the ones the manifest gives, that the
> backend is the manifest backend, and that the `hello` carries a runner identity
> whose `manifest_sha256` is the canonical digest of the manifest. Each failed
> check has its own name in the report. The checks are audit only. No result of
> these checks changes a score.
...
One `docker compose up` after `benchd transform` populates the weights volume.
...
> **STATUS: DONE.** Every phase below has landed. Phase 0's `PROTOCOL.md` + JSON Schema
> are frozen; the engine is extracted into its own repo and consumed as a SHA-pinned
> submodule the other way round (the engine repo pins benchd — see
> `docs/track-release-branches.md`); benchd reached and passed MLX parity (closed
> 2026-08-20); the CUDA engine is in bring-up on the ruled RTX PRO 6000 Blackwell box.
> Phase 5's `benchmark.sh` retirement did NOT happen as written: `scripts/benchmark.sh`
> was deliberately KEPT as the harness root the engine repo's `benchmark.json` invokes,
> and its hash is load-bearing. Kept as the record of the sequencing.
...
4. **Phase 3 — CUDA engine bring-up**: container passes the correctness + anchor
   ...
   then timed runs.
5. **Phase 4 — cut over**: official pipeline invokes `benchd official`; retire
...
## 9. Risks and open questions — RESOLVED / RULED
