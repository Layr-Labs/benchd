...
**Legacy path.** This gate belongs to the operator-driven GPU windows of the 27B track's M5
boxes (parity re-verifies, Proof A, measure-job windows), where a serving model has to be
unloaded and a session lock taken by hand. A Yukon-dispatched ranked job on the 125B tracks
takes `/tmp/mtplx-gpu-exclusive.lock` inside the job and runs the engine repo's own preflight
(`tools/ranked-box-preflight.sh`); it does not run this script.

**Every operator-driven GPU window runs `scripts/window-preflight.sh` before the lock. No exceptions.**
...
A miniature of the **real** path: the actual `benchd` binary spawning the actual worker
...
There is deliberately **no new `benchd smoke` subcommand**: adding one would mean editing the
...
An `iterate` or `correctness` run that gives `--engine-resource NAME=PATH` (repeatable) adds one
`--resource NAME=PATH` for each, before `--speculative-protocol`:

```
<worker> runtime-worker --weights W --resource qwen4exp.ngramRowSource=D --speculative-protocol v1.1
```

The runner needs a resource to **load** the model, so every spawn of the run carries it, including
the teacher-forced correctness spawn that omits `--speculative-protocol`. The value comes from
benchd's own command line. benchd checks the name and the shape only; the worker checks that the
path exists and has the shape the resource needs, and refuses by name.
...
| shape check | handed to `benchd validate-golden` — the same loader the run uses | required-key **signature** (`seed_tokens`, `reference_seed_token`, `rows`) — what `measure-job` actually routes on |
...
`attach-benchmark-oracle` remedy (`crates/benchd/src/measure_job.rs`,
...
in the probe reproduces `crates/benchd/src/iterate.rs:180-212` exactly, including the
...
**Build `benchd` first** (`cargo build --release -p benchd`). The real-transport sections need
...
* the **smoke leg against fake workers driven by the real `benchd` over the real transport** —
...
**A SKIP fails the suite.** An unbuilt `benchd` used to substitute an always-pass stub, so the
