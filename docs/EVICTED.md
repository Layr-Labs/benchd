...
|---|---|---|---|
| `docs/measure-job-contract.md` | `cd5782e` (#155) | `fe2da64` | **Normative in code**: `crates/benchd/src/measure_job.rs` (paired flow, retry, rejection classes), and in [`model2-calibration.md`](model2-calibration.md). Surviving cites are pinned `@fe2da64` (final revision) or `@7c6be14` (the WS2-5 revision the line-numbered cites were written against). |
| `docs/paired-flow-design-note.md` | `cd5782e` (#155) | `3af70c8` | **Implemented**: the three seams are `scripts/official-paired.sh` (driver), `benchd measure-job` (A-1), the gates producer (A-2), `benchd overlay-timing` (A-3). Surviving cites pinned `@3af70c8`. |
| `docs/parity-matrix.md` | `fe2da64` (#159) | `fe2da64^` | **`crates/benchd/tests/fixtures/waiver-ledger.json`** — the §13 waiver rows as structured data, read directly by `parity.rs`'s sign-off test. `docs/parity-completion-gate.md`'s `@35c100a` cites resolve unchanged via git. |
...
`crates/benchd/tests/fixtures/swift-official-baseline-constants.json` carries an unpinned
