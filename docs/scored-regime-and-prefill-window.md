...
**Scope.** This page governs the `measure-job` → `overlay-timing` seam and the scored regime
the track fixture declares for it (today: `qwen3.8-27b-mtp-v1` only). The
`iterate --mode official` path is a different seam: it has always sealed
`prefill_speedup^0.25 * decode_speedup^0.75` by the Swift-parity formula, and on the Qwen 3.8
125B-A6B tracks it does so over a live control leg
([`architecture.md`](architecture.md) §1). The standing limit in part 3 is a limit on this
seam's declarations, not on that path.
...
Part 3 is a normative limit on part 1 for the tracks that score through this seam: none of them
may declare a nonzero prefill exponent yet.

Parts 1 and 2 are one mechanism. The declaration decides whether the certification is armed.

**Scope.** This page governs the `measure-job` paired flow and the tracks that declare a scored
regime. The Qwen 3.8 125B-A6B tracks declare no regime: they measure their own denominator and
score on the paired per-box path — see
...
The regime is declared in the track fixture, beside the official baseline pair. The three values
are ONE declaration: the fixture states all three or none. `TRACK_ID` in
`crates/bench-core/src/constants.rs` names the tree's own track.

Read the regime with `bench_core::contract::scored_regime(contract, track_id)`. It is the only
accessor. A fixture that declares none of the three values declares no regime. That state is
`SCORED_REGIME_PENDING`. The accessor refuses it, and the refusal names the `track_id`, the
`SCORED-REGIME-PENDING-DECLARATION` sentinel and the three keys. A fixture that declares one
exponent without the others is refused when it is parsed.

There is no table and no placeholder. There is no exponent pair for a new track to inherit by
accident.

### What the 27B fixture declares
...
The two Qwen 3.8 125B-A6B tracks declare NO regime. They declare
`scores_against_live_control_leg: true`, so `run_baselines` takes the live-control-leg arm before
`declares_a_scored_regime` is asked, and this fence never sees them. Their composite comes from
...
The exponents that pin that identity are fixture data. Change either one and the composite the
track scores changes with it.
...
has three sources and only the last one reads the fixture, so a fence inside the resolution would
be stepped around by an environment pair or by a golden that declares a pair. The fence outside it
is not.
...
2026-08-27. They declare no regime here, and this limit does not govern them.
